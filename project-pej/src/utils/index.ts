...
// // 2024-01-18
// //! 토큰 token & 쿠키cookie
// // : 웹 상에서, 정보를 저장 | 전송하는 방법들
// // : http 통신(post get put delete)에서 해당 정보를 저장 가능

// //? 토큰 token
// // : 사용자 인증 및 권한 부여에 사용되는 암화화된 문자열
// // : 서버에서 사용자 인증 진행 → 해당 유저에 대한 세션 정보를 나타냄
// // ex. 유저가 로그인 > 서버가 사용자의 인증 정보를 확인, >>
// // >> 토큰 생성 > 생성된 토큰 사용 & 요청에 자신을 인증
// // >> (http 헤더에 포함하여 서버로 전송)

// //? 쿠키 cookie
// // : 웹 사이트가 유저의 브라우저에 저장하는 작은 데이터 조각
// // : 유저의 세션 관리, 개인화 설정 | 추적 etc 사용
// // ex. 유저가 웹 사이트 방문 > 사이트: 쿠키 생성, 브라우저 저장 >>
// // >> 이후 유저가 해당 사이트를 방문 > 브라우저: 쿠키 정복를 사이트에 전달 >>
// // >> 사용자를 식별하게 됨
// // ex. 네이버: 로그인 상태 유지
